...
function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-4" aria-hidden="true">
      ...
      <path
        fill="#34A853"
        d="M12 24c3.2 0 5.9-1.1 7.9-2.9l-3.7-2.8c-1 .7-2.4 1.2-4.2 1.2-3.2 0-6-2.2-7-5.1L1.2 17.1C3.2 21.2 7.3 24 12 24z"
      />
      <path
        fill="#FBBC05"
        d="M5 14.4c-.2-.7-.4-1.5-.4-2.4s.1-1.7.4-2.4L1.2 6.9C.4 8.5 0 10.2 0 12s.4 3.5 1.2 5.1l3.8-2.7z"
      />
      <path
        fill="#EA4335"
        d="M12 4.7c1.8 0 3 .8 3.7 1.4l3.3-3.2C17.9 1.1 15.2 0 12 0 7.3 0 3.2 2.8 1.2 6.9L5 9.6c1-2.9 3.8-4.9 7-4.9z"
      />
    </svg>
  );
}
      ...
      <aside className="hidden flex-col justify-between bg-inkblue/10 p-12 lg:flex">
        ...
        <div>
          ...
          <p className="mt-4 max-w-md leading-7 text-muted-foreground">
            A guided path from private manuscript to published book, with a calm coach beside you
            the whole way.
          </p>
          <ul className="mt-8 space-y-3 text-sm">
            {[
              "Six phases, paced around your launch date",
              "One clear next step at a time",
              "Your editors and readers in one place",
            ].map((line) => (
              <li key={line} className="flex items-center gap-3">
                <span className="grid size-6 place-items-center rounded-full bg-leaf/25">
        ...
        <p className="text-xs text-muted-foreground">
          For independent authors and the people who help them.
        </p>
      </aside>
        ...
        <div className="w-full max-w-md">
          <div className="mb-8 lg:hidden">
            <Link
              ...
              aria-label="The Indie Book Table"
            >
              <img
                src={falconAsset.url}
                alt=""
                ...
                height={2000}
              />
              <span className="font-serif text-xl font-normal">The Indie Book Table</span>
            </Link>
          </div>
            ...
            {(
              ...
              <button
                ...
                className={cn(
                  ...
                  mode === value ? "bg-card shadow-xs" : "text-muted-foreground",
                )}
              >
                {label}
              </button>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {mode === "signin"
              ? "Pick up right where your book left off."
              : "Your shelf, your cycles, and any book shared with you."}
          </p>
          ...
          >
            {mode === "signup" && (
              <>
                <p className="rounded-xl border border-border bg-card p-4 text-xs leading-5 text-muted-foreground">
                  One account covers everything: write your own books, and accept invitations to
                  help on other authors&rsquo; books.
                </p>
            ...
            {error && (
              <p className="rounded-xl border border-clay/50 bg-clay/12 p-3 text-sm">{error}</p>
            )}
            {forgotSent && (
              <p className="rounded-xl border border-teal/40 bg-teal/12 p-3 text-sm">
