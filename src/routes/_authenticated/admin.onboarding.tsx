...
type StepDraft = Omit<WorkshopTourStep, "created_at" | "updated_at">;
...
function AdminOnboarding() {
  ...
  return <div className="space-y-8">
    <section className="rounded-lg border border-border bg-card p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-serif text-2xl font-normal">First-visit welcome</h2><p className="mt-1 text-sm text-muted-foreground">Shown when an author first enters the Workshop.</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => window.dispatchEvent(new Event(OPEN_WORKSHOP_WELCOME))}><Eye />Preview</Button><label className="flex items-center gap-2 text-sm"><Switch checked={currentWelcome.enabled} onCheckedChange={(enabled) => updateWelcome({ enabled })} />{currentWelcome.enabled ? "On" : "Off"}</label></div></div>
    ...
    <section>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-serif text-2xl font-normal">Walkthrough screens</h2><p className="mt-1 text-sm text-muted-foreground">Order and publish the screens authors see.</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => window.dispatchEvent(new Event(OPEN_WORKSHOP_TOUR))}><Eye />Preview published</Button><Button onClick={() => setAdding(true)}><Plus />Add screen</Button></div></div>
      <div className="space-y-4">
        {adding ? <StepEditor initial={{ id: crypto.randomUUID(), title: "", body: "", media_kind: "none", media_value: "", destination_label: "", destination_path: "", position: steps.length, published: false }} onSaved={refresh} /> : null}
        {steps.map((step, index) => <div key={step.id} className="grid gap-2 sm:grid-cols-[40px_minmax(0,1fr)]"><div className="flex gap-1 sm:flex-col"><Button variant="ghost" size="icon" disabled={index === 0 || move.isPending} onClick={() => { const ids = steps.map((item) => item.id); [ids[index - 1], ids[index]] = [ids[index], ids[index - 1]]; move.mutate(ids); }} aria-label="Move screen up"><ArrowUp /></Button><Button variant="ghost" size="icon" disabled={index === steps.length - 1 || move.isPending} onClick={() => { const ids = steps.map((item) => item.id); [ids[index], ids[index + 1]] = [ids[index + 1], ids[index]]; move.mutate(ids); }} aria-label="Move screen down"><ArrowDown /></Button></div><StepEditor initial={step} onSaved={refresh} onDelete={() => remove.mutate(step.id)} /></div>)}
      </div>
    </section>
  </div>;
}