      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {templates.map((template, index) => (
          <article key={template.id} className="grid overflow-hidden rounded-2xl border border-border bg-card shadow-xs transition-shadow hover:shadow-md sm:grid-cols-[180px_1fr]">
            <img src={template.cover} alt={`Cover artwork for the ${template.name}`} width={768} height={1152} className="h-full min-h-64 w-full object-cover" loading={index === 0 ? undefined : "lazy"} />
            <div className={index === 0 ? "bg-amber/8 p-6" : "bg-teal/8 p-6"}>
              ...
              <h2 className="font-serif text-3xl font-normal">{template.name}</h2>
