...
function Index() {
  return (
    <AppShell>
      ...
      <section className="mb-10 grid gap-3 sm:grid-cols-3" aria-label="Publishing priorities">
        <div className="border-l-4 border-destructive bg-card px-4 py-3"><div className="flex items-center gap-2 text-sm font-semibold"><CircleAlert className="size-4 text-destructive" />Behind pace</div><p className="mt-1 text-2xl font-semibold">1</p></div>
        <div className="border-l-4 border-accent bg-card px-4 py-3"><div className="flex items-center gap-2 text-sm font-semibold"><CalendarDays className="size-4" />Launch approaching</div><p className="mt-1 text-2xl font-semibold">1</p></div>
        <div className="border-l-4 border-primary bg-card px-4 py-3"><div className="flex items-center gap-2 text-sm font-semibold"><Clock3 className="size-4 text-primary" />Pending action</div><p className="mt-1 text-2xl font-semibold">2</p></div>
      </section>
      <section>
        ...
        <div className="divide-y divide-border border-y border-border">
          {books.map((book) => (
            <Link key={book.id} to="/books/$bookId" params={{ bookId: book.id }} className="group grid gap-5 bg-card px-4 py-5 transition-colors hover:bg-secondary/60 sm:grid-cols-[88px_1fr_auto] sm:items-center">
              <img src={book.cover} alt={`Cover artwork for ${book.title}`} width={768} height={1152} className="aspect-[2/3] w-20 object-cover shadow-sm" loading="lazy" />
              <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-serif text-2xl font-semibold group-hover:text-primary">{book.title}</h3><StatusPill tone={book.progress > 60 ? "warm" : "good"}>{book.status}</StatusPill></div><p className="mt-1 text-sm text-muted-foreground">{book.genre} · by {book.author}</p><div className="mt-4 flex max-w-xl items-center gap-3"><Progress value={book.progress} className="h-1.5" /><span className="text-xs font-semibold">{book.progress}%</span></div><p className="mt-3 text-sm"><span className="text-muted-foreground">Next:</span> {book.nextAction}</p></div>
